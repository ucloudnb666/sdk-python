...
class TestEstimateTokens:
    """Tests for AnthropicModel._estimate_tokens native token counting."""
    ...
    @pytest.mark.asyncio
    async def test_native_token_count_success(self, model_with_client, anthropic_client, messages):
    ...
    @pytest.mark.asyncio
    async def test_native_token_count_with_system_prompt(self, model_with_client, anthropic_client, messages):
        """System prompt is passed to count_tokens."""
        mock_response = unittest.mock.MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_native_token_count_with_system_prompt_content(self, model_with_client, anthropic_client, messages):
        """System prompt content blocks are joined into a string for count_tokens."""
        mock_response = unittest.mock.MagicMock()
        mock_response.input_tokens = 60
        anthropic_client.messages.count_tokens = unittest.mock.AsyncMock(return_value=mock_response)

        result = await model_with_client._estimate_tokens(
            messages=messages,
        ...
        assert result == 60
        call_kwargs = anthropic_client.messages.count_tokens.call_args[1]
        assert call_kwargs["system"] == "Be helpful. Be concise."

    @pytest.mark.asyncio
    async def test_native_token_count_with_tool_specs(self, model_with_client, anthropic_client, messages, tool_specs):
    ...
    async def test_max_tokens_stripped_from_request(self, model_with_client, anthropic_client, messages):
        """max_tokens is not passed to count_tokens."""
        mock_response = unittest.mock.MagicMock()
