...
class TestEstimateTokens:
    """Tests for LlamaCppModel._estimate_tokens native token counting."""
    ...
    @pytest.mark.asyncio
    async def test_native_token_count_success(self, model, messages):
    ...
    @pytest.mark.asyncio
    async def test_native_token_count_with_system_prompt(self, model, messages):
        """System prompt is included in the tokenize request."""
        mock_response = MagicMock()
        mock_response.status_code = 200
    ...
    @pytest.mark.asyncio
    async def test_native_token_count_with_tool_specs(self, model, messages, tool_specs):
    ...
    @pytest.mark.asyncio
    async def test_native_token_count_with_system_prompt_content(self, model, messages):
        """System prompt content blocks are joined into a string."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"tokens": list(range(15))}
        mock_response.raise_for_status = MagicMock()
        model.client.post = AsyncMock(return_value=mock_response)
    ...
    async def test_fallback_on_http_error(self, model, messages):
        """Falls back to base estimation when HTTP request fails."""
    ...
    async def test_fallback_on_connection_error(self, model, messages):
        """Falls back to base estimation when server is unreachable."""
